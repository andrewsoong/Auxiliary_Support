...
"""
Usage:
    calc_oneday_global_drawmap.py --sat=mysat --ins=myins --just_calc=false
...
systime= datetime.datetime.now() + datetime.timedelta(days=-4)
...
#         return True
    ...
    for one_table in my_table:
        try:
            ...
            sql = 'select count(id) from ' + one_table + ' where obs_bt' + str(channel)+  ' != -999999'
            print sql
    ...
    sub_rttov_select = 'select ymdhms, lat, lon, obs_bt' + str(channel) + ', rttov_bt' \
                + str(channel) + ' '
    total = ' ) as total ' + 'limit ' + str(scans*pixels) + ' '
#     sql_rttov = sql_rttov_temp + ' union all '.join(map(lambda x,y: x + ' from ' + y, \
#                 [sub_rttov_select]*len(my_table), my_table) ) + total
    sql_rttov = sql_rttov_temp +' union all '.join(map(lambda x,y: x + ' from ' + y + ' where obs_bt' + str(channel) + ' != -999999 ', \
                [sub_rttov_select]*len(my_table), my_table) ) + total 
...
#                     + 'rttov.' + common.utc_YmdHMS() + '.txt'
    ...
    sub_crtm_select = 'select ymdhms, lat, lon, obs_bt' + str(channel) + ', crtm_bt' \
                + str(channel) + ' '
    total = ' ) as total ' + 'limit ' + str(scans*pixels) + ' '
    sql_crtm = sql_crtm_temp + ' union all '.join(map(lambda x,y: x + ' from ' + y + ' where obs_bt' + str(channel) + ' != -999999 ', \
...
#         return True
    ...
    #map_ncl = conf.plot_path + '/global_map.ncl'
