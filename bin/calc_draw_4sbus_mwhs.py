...
#     get_obc_txt(tmpfile1year,tmpfilelife,'scan_prd')
    ...
    print len (data_3day)
    print "################################"
    print len (ymdh_arr.astype(numpy.int32))
    
    ret = create_obc_hdf('3day',hfile,data_3day, ins_conf.obc_to_db.values())
    
    ymdh_arr = numpy.array(map(common.time_to_arr, data_1month_calc[:, 0]) )
    hfile.create_dataset("time_1month_calc", data = ymdh_arr.astype(numpy.int32))
    ret = create_obc_hdf('1month_calc',hfile,data_1month_calc, ins_conf.calc_to_db.values())

    ymdh_arr = numpy.array(map(common.time_to_arr, data_3day_calc[:, 0]) )
    hfile.create_dataset("time_3day_calc", data = ymdh_arr.astype(numpy.int32))  
    ret = create_obc_hdf('3day_calc',hfile,data_3day_calc, ins_conf.calc_to_db.values())
    print "################################"
    print len (data_3day_calc)
    if not ret:
        return False
...
def main():
    ...
    #now_time = datetime.datetime.now()
    now_time = datetime.now() + timedelta(days = 17)
...
#         sys.exit(4)
    ...
    if int(shifen)< 0030:
        for i in range(0, 3):
            ...
            ymd = yes_time.strftime('%Y%m%d')
            ymd = '201411'+ymd[6:8]
            for idx, one_table in enumerate(201411):
                if ymd in one_table:
