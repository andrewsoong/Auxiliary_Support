...
calculate_table_3day = []
calculate_table_1month = []
...
def draw_just_obc():
    ...
    if len(obc_table_1month) <= 0:
        return True
    if len(calculate_table_3day) <= 0:
        return True
    if len(calculate_table_1month) <= 0:
        return True
    
    
    tmpfileC3day = '/assimilation/fymonitor/DATA/TMP/' + my_tag + '.' + common.utc_YmdHMS() +'.C3day' +'.obc'
    sql_C3day = common.get_obc_2dim_sql(ins_conf.calc_to_db.values(),ins_conf.channels,
                                  calculate_table_3day, conf.obc_select_prefix_sql) \
        + conf.export_txt%(tmpfileC3day + '.txt')
      
    
    
    tmpfileC1month = '/assimilation/fymonitor/DATA/TMP/' + my_tag + '.' + common.utc_YmdHMS() +'.C1month' +'.obc'
    sql_C1month = common.get_obc_2dim_sql(ins_conf.calc_to_db.values(),ins_conf.channels,
                                  calculate_table_1month, conf.obc_select_prefix_sql) \
        + conf.export_txt%(tmpfileC1month + '.txt')
    ...
    try:
        ...
        cur.execute(sql_3day)
        cur.execute(sql_1month)
        cur.execute(sql_C3day)
        cur.execute(sql_C1month)
        cur.execute(begin_sql_3day)
        begin_data = cur.fetchall()
        cur.close()
        conn.close()
    ...
    data_3day = numpy.loadtxt(tmpfile3day + '.txt', dtype='str', delimiter=',')
    data_1month = numpy.loadtxt(tmpfile1month + '.txt', dtype='str', delimiter=',')
    data_C3day = numpy.loadtxt(tmpfileC3day + '.txt', dtype='str', delimiter=',')
    data_C1month = numpy.loadtxt(tmpfileC1month + '.txt', dtype='str', delimiter=',')
    ...
    ymdh_arr = numpy.array(map(common.time_to_arr, data_C1month[:, 0]) )
    hfile.create_dataset("time_C1month", data = ymdh_arr.astype(numpy.int32))     
    ret = create_obc_hdf('C1month',hfile,data_C1month, ins_conf.calc_to_db.values())
    
    ymdh_arr = numpy.array(map(common.time_to_arr, data_C3day[:, 0]) )
    hfile.create_dataset("time_C3day", data = ymdh_arr.astype(numpy.int32))     
    ret = create_obc_hdf('C3day',hfile,data_C3day, ins_conf.calc_to_db.values())
...
def main():
    ...
    calculate_table=[ x for x in all_obc_table if calculate_tag in x[0]]
    obc_table = list(set(all_obc_table).difference(set(channel_table))) #return in all_obc_table but no in channel_table
    obc_table = list (set(obc_table).difference(set(calculate_table)))
    for idx, one_table in enumerate(calculate_table):
        ...
        if timespan['begin_t'] == cur_timeStamp:
            my_calculate_table.extend([one_table[0]])
    ...
    if int(shifen)< 0030:
        for i in range(0, 3):
            ...
            for idx, one_table in enumerate(my_calculate_table):
                if ymd in one_table:
                    calculate_table_3day.extend([one_table])
    else:
        for i in range(0, 3):
    ...
    for i in range(0, 30):
        ...
        for idx, one_table in enumerate(my_calculate_table):
            if ymd in one_table:
                calculate_table_1month.extend([one_table])  
