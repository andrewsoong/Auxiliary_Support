...
def draw_one_channel_crtm(channel):
    ...
    point_data = numpy.arange(1, 90 + 1, int(1), dtype = numpy.int32)
    ...
    cmd = conf.ncl + ' ' + " 'sat=\"" \
        ...
        + " 'file_in=\"" + tmphdf + "\"' " \
        + " 'file_out=\"" + tmp_png + "\"' " \
        + " 'file_title=\"" + png_title + "\"' "  + " /home/fymonitor/MONITORFY3C/py2/plot/fy3c_mwts_scan_point_time_crtm.ncl"\
        + ' > ' + tmpfile + '.log' + ' 2>&1'
    print cmd
...
def draw_one_channel_rttov(channel):
    ...
    cmd = conf.ncl + ' ' + " 'sat=\"" \
        ...
        + " 'file_in=\"" + tmphdf + "\"' " \
        + " 'file_out=\"" + tmp_png + "\"' " \
        + " 'file_title=\"" + png_title + "\"' "  + " /home/fymonitor/MONITORFY3C/py2/plot/fy3c_mwts_scan_point_time_rttov.ncl"\
        + ' > ' + tmpfile + '.log' + ' 2>&1'
    print cmd
    (status, output) = commands.getstatusoutput(cmd)
    common.debug(my_log, log_tag, str(status) + '`' + cmd + '`' + output)
...
def check_calc_one_channel(channel):
    for i in range(1,5):
        if check_one_channel(channel) != 180:
