...
Usage:
    calc_draw_obc.py --sat=fy3c --ins=mwts --span=12 --date=now --just_calc=false
...
def draw_one_channel(channel):
    ...
    try:
        ...
        msg = 'clean tmp txt error[' + str(e.args[0])+']: ' + e.args[1]              
...
def draw_channel(tmphdf, channel, png_title, begin_time, end_time):
    ...
    for i in xrange(0, sds_len): 
        ...
        temp_cmd = conf.ncl + " 'sat=\"" + sat.upper() + "\"' " \
            ...
            + " 'file_in=\"" + tmphdf + "\"' " \
            + " 'file_out=\"" + file_out + "\"' " \
            + " 'file_title=\"" + png_title +'_'+conf.draw_ncl[ins]['ncl_prog_channel'][i]['tmp_png'].upper()+ "\"' " +  file_title \
            + ' > ' + temp_log + ' 2>&1'
        print temp_cmd
        cmd.append(temp_cmd)
...
#     pooltest.join()
    ...
    for i in xrange(0, sds_len): 
        ...
        dest_path = '/hds/assimilation/fymonitor/DATA/IMG/NSMC/' + sat.upper() + '/' + ins.upper() + '/' \
                + conf.draw_ncl[ins]['ncl_prog_channel'][i]['dir_name'] + '/'
        ...
        shutil.copyfile(file_out + '.png', arch_path + png_title + '_' + conf.draw_ncl[ins]['ncl_prog_channel'][i]['tmp_png'].upper() + '.png')
        common.empty_folder(latest_path)
        common.mv_file(file_out + '.png', latest_path + png_title + '_' + conf.draw_ncl[ins]['ncl_prog_channel'][i]['tmp_png'].upper() + '.png')
...
#             return False
    ...
    try:
        ...
        msg = 'clean tmp hdf error[' + str(e.args[0])+']: ' + e.args[1]              
        common.warn(my_log, log_tag, time_tag + msg)
...
def draw_nochannel(tmphdf, png_title, begin_time, end_time):    
    cmd = []
    for i in xrange(0, len(conf.draw_ncl[ins]['ncl_prog_no_channel'])): 
        ...
        temp_cmd = conf.ncl + " 'sat=\"" + sat.upper() + "\"' " \
            ...
            + " 'file_in=\"" + tmphdf + "\"' " \
            + " 'file_out=\"" + file_out + "\"' " \
            + " 'file_title=\"" + png_title +conf.draw_ncl[ins]['ncl_prog_no_channel'][i]['tmp_png'].upper()+ "\"' " +  file_title \
            + ' > ' + temp_log + ' 2>&1'
        print temp_cmd
        cmd.append(temp_cmd)
...
#     pooltest.join()
    ...
    for i in xrange(0, len(conf.draw_ncl[ins]['ncl_prog_no_channel'])): 
        ...
        dest_path = conf.img_nsmc + '/' + sat.upper() + '/' + ins.upper() + '/' \
                  + conf.draw_ncl[ins.lower()]['ncl_prog_no_channel'][i]['dir_name'] + '/' 
                  #+ conf.png_type[conf.draw_ncl[ins]['ncl_prog_no_channel'][i]['tmp_png']] + '/'
        arch_path = dest_path + str(end_time[0:4]) + '/'
        latest_path = dest_path + 'LATEST/' 
        shutil.copyfile(file_out + '.png', arch_path + png_title +conf.draw_ncl[ins]['ncl_prog_no_channel'][i]['tmp_png'].upper()+ '.png')
        common.empty_folder(latest_path)
        common.mv_file(file_out + '.png', latest_path + png_title +conf.draw_ncl[ins]['ncl_prog_no_channel'][i]['tmp_png'].upper()+ '.png')
        os.remove(file_out + '.png.OK')
        os.remove(temp_log)
#         try:
#             shutil.copyfile(file_out + '.png', arch_path + png_title +conf.draw_ncl[ins]['ncl_prog_no_channel'][i]['tmp_png'].upper()+ '.png')
