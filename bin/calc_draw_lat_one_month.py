...
#         + " where channel=" + str(channel) + conf.export_txt%(tmpfile + '.txt')
    sql = 'select num, avg, std from ' + stat_table + " where ymdh like '%2014-11%' and channel=" \
        + str(channel) #+ ' order by sim_mod'
    
    sql_time = 'select ymdh from ' + stat_table + " where ymdh like '%2014-11%' and channel=" \
...
#                 + 'H_' + lat_span + 'LAT_' # RTTOV, CRTM
    png_title = 'FY3C_MWTS_20141101_TO_20141130_'+'CH'+  format(channel, '02d') +'_06H'   
