...
Usage:
    calc_draw_lat_by_point_mwri.py --sat=fy3c --ins=mwts --nwp=t639 --lat_span=5
...
# --sat=fy3c --ins=mwts --nwp=t639 --lat_span=5 --hour_span=6 --date=now
...
def draw_one_channel_crtm(channel):
    ...
    cmd = conf.ncl + ' ' + " 'sat=\"" \
        ...
        + " 'file_in=\"" + tmphdf + "\"' " \
        + " 'file_out=\"" + tmp_png + "\"' " \
        + " 'file_title=\"" + png_title + "\"' "  + " /home/fymonitor/MONITORFY3C/py2/plot/fy3c_mwri_scan_point_time_crtm.ncl"\
        + ' > ' + tmpfile + '.log' + ' 2>&1'
    print cmd
...
def draw_one_channel_rttov(channel):
    ...
    cmd = conf.ncl + ' ' + " 'sat=\"" \
        ...
        + " 'file_in=\"" + tmphdf + "\"' " \
        + " 'file_out=\"" + tmp_png + "\"' " \
        + " 'file_title=\"" + png_title + "\"' "  + " /home/fymonitor/MONITORFY3C/py2/plot/fy3c_mwri_scan_point_time_rttov.ncl"\
        + ' > ' + tmpfile + '.log' + ' 2>&1'
    print cmd
    (status, output) = commands.getstatusoutput(cmd)
    common.debug(my_log, log_tag, str(status) + '`' + cmd + '`' + output)
...
def calc_one_channel(channel):
    rttov_sql = common.get_sql_calc_stat(channel, ins_conf.sql_rttov_by_point,
                                         my_table)
    common.debug(my_log, log_tag, 'channel '+str(channel)+'`rttov`'+rttov_sql)

    crtm_sql = common.get_sql_calc_stat(channel, ins_conf.sql_crtm_bt_point,
                                        my_table)
    ...
    try:
        ...
        for point in range(1,ins_conf.pixels+1):      
            ...
            sql = rttov_sql%((point,)*len(my_table))
            ...
            sql = crtm_sql%((point,)*len(my_table)) 
...
def main():
    ...
    if len(my_table)<=0:
        ...
        sys.exit(4)
    calc_draw_one_channel(8)
    calc_draw_one_channel(9)
    calc_draw_one_channel(10)
    return
