...
"""Temporary program, calculate for someday by calling calc_draw_bt.py
finally, create png at end day. Make sure there are data in db for end_t, else
there will NO png at all. 

Usage:
    calc_for_someday_bt.py --sat=fy3c --ins=mwts --nwp=t639 --span=6
...
#    for each y-m-d-h:
#        calling calc_draw_bt.py
#    for the last y-m-d-h, calling calc_draw_bt.py with draw=True
#
# /home/fymonitor/python27/bin/python
# /home/fymonitor/MONITORFY3C/py2/bin/calc_for_someday_bt.py 
# --sat=fy3c --ins=mwts --nwp=t639 --span=6 --begin_t=2014-01-01 --end_t=2014-01-02
# >> /home/fymonitor/DATA/LOG/calc_for_someday_bt.py.log 2>&1
#                         
# date          author    changes
# 2014-07-25    gumeng    create
...
while cur_ts <= end_timestamp:
    for one_span in span_setting[hour_span]:
        ...
        cmd_prefix = 'source ~/.bashrc; ' + conf.python + ' ' + conf.bin_path \
                    + '/calc_draw_bt.py --sat=' + sat + ' --ins=' + ins \
                    + ' --nwp=' + nwp+ ' --span=' + hour_span \
