...
def check_calc_one_channel(channel):
    for i in range(1,2):
        print 'DB should have: '
        print (ins_conf.pixels * 2)
...
def check_one_channel(channel):
    sql = "select count(*) from " + stat_table +" where channel=" + str(channel) +" and ymdh like '"\
            + timespan['end_str'] +"';"
    delete_sql = "delete from " + stat_table +" where channel=" + str(channel) +" and ymdh like '"\
            + timespan['end_str'] +"';"
    ...
    try:
        ...
        common.err(my_log, log_tag, time_tag + msg)
        return False 
    print 'but db really have :'
    print num[0][0]

    if num[0][0] != (ins_conf.pixels * 2):
        print 'I will delete data !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
        print delete_sql
        try:
            conn=MySQLdb.connect(host=conf.db_setting['master']['ip'], 
                                 user=conf.db_setting['master']['user'],
