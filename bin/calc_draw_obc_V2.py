...
Usage:
    calc_draw_obc.py --sat=fy3c --ins=mwts --span=12 --date=now --delhdf=True
...
def draw_channel(tmphdf, channel, png_title, begin_time, end_time):
    ...
    for i in xrange(0, sds_len): 
        ...
        temp_cmd = conf.ncl + " 'sat=\"" + sat.upper() + "\"' " \
            ...
            + " 'file_in=\"" + tmphdf + "\"' " \
            + " 'file_out=\"" + file_out + "\"' " \
            + " 'file_title=\"" + png_title + '_' + conf.draw_ncl[ins]['ncl_prog_channel'][i]['dir_name'] + "\"' " +  file_title \
            + ' > ' + temp_log + ' 2>&1'
        print temp_cmd
        cmd.append(temp_cmd)
...
def draw_nochannel(tmphdf, png_title, begin_time, end_time):    
    ...
    for i in xrange(0, sds_len): 
        ...
        temp_cmd = conf.ncl + " 'sat=\"" + sat.upper() + "\"' " \
            ...
            + " 'file_in=\"" + tmphdf + "\"' " \
            + " 'file_out=\"" + file_out + "\"' " \
            + " 'file_title=\"" + png_title + '_' + conf.draw_ncl[ins]['ncl_prog_no_channel'][i]['dir_name'] + "\"' " +  file_title \
            + ' > ' + temp_log + ' 2>&1'
        #print temp_cmd
        cmd.append(temp_cmd)
