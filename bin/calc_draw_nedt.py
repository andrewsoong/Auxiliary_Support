...
Usage:
    calc_draw_nedt.py --sat=fy3c --ins=mwts --span=12 --date=now
...
def draw_one_channel(channel):
    ...
    sql = 'select ymdh , nedn from ' + sat.upper() + '_' + ins.upper() + \
        '_NEDN where channel=' + str(channel) + ' INTO OUTFILE ' + '\'' \
        + tmpfile + '.txt' + '\'' + '  FIELDS TERMINATED BY \',\' '
    begin_sql = 'select ymdh , nedn from ' + sat.upper() + '_' + ins.upper() + \
        '_NEDN where channel=' + str(channel) + " limit 1"
...
def draw_channel(tmphdf, channel, png_title, begin_time, end_time):
    file_out = conf.plot_path + '/'+ png_title + '_' + 'NEdT'
    file_title = conf.plot_path +'/FY3C_NEdT.ncl'
    temp_log = conf.tmp_path + '/monitor.' + log_tag + '.ch' + channel+'.' +'Sensitivity.log'
    temp_cmd = conf.ncl + " 'sat=\"" + sat.upper() + "\"' " \
        + "'instrument=\"" + ins.upper() + "\"' channel=" + str(channel) \
        + ' \'' +'BG_YMD=' + begin_time + '\' ' + ' \'' + 'ED_YMD=' + end_time + '\''\
        + " 'file_in=\"" + tmphdf + "\"' " \
        + " 'file_out=\"" + file_out + "\"' " \
        + " 'file_title=\"" + png_title + '_NEdT' + "\"' " +  file_title \
        + ' > ' + temp_log + ' 2>&1'
    print temp_cmd
