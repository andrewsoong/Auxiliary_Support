...
Usage:
    calc_draw_4sbus_iras.py --sat=fy3c --ins=iras --span=12 --date=now
...
#                             numpy_data[:, idx].astype(numpy.float32))
                    ...
                    and 'int' in one_setting['db_dtype']:
                    hfile.create_dataset(ds_name, data = \
                            numpy_data[:, idx].astype(numpy.float32)/1000)
...
def getcalc_sql(tables,channel,field):
    prefix ='select * from ('
    subsql = 'select * from %s where channel=%s and type= \'%s\''
    postfix_sql = ') as total'
    subsql_total=''
    for idx, one_table in enumerate(tables):
        if idx == 0:
            subsql_total =  subsql%(one_table,channel,field) + subsql_total
        else:
            subsql_total =  subsql%(one_table,channel,field) + ' union all ' +subsql_total
...
def draw_one_channel(channel):
    ...
    blackbody_view_tmpfile3day = '/assimilation/fymonitor/DATA/TMP/' + my_tag + '.' + common.utc_YmdHMS() \
            + '.ch' + format(channel, '02d') + '.blackbody_view' +'.3day' 
    blackbody_view_tmpfile1month = '/assimilation/fymonitor/DATA/TMP/' + my_tag + '.' + common.utc_YmdHMS() \
            + '.ch' + format(channel, '02d')+ '.blackbody_view' + '.1month'  
    blackbody_view_sql_3day = getcalc_sql(Calc_channel_table_3day,str(channel),'blackbody_view')+ conf.export_txt%(blackbody_view_tmpfile3day + '.txt')
    blackbody_view_sql_1month = getcalc_sql(Calc_channel_table_1month,str(channel),'blackbody_view')+ conf.export_txt%(blackbody_view_tmpfile1month + '.txt')

    coldspace_view_tmpfile3day = '/assimilation/fymonitor/DATA/TMP/' + my_tag + '.' + common.utc_YmdHMS() \
            + '.ch' + format(channel, '02d') + '.coldspace_view' +'.3day' 
    coldspace_view_tmpfile1month = '/assimilation/fymonitor/DATA/TMP/' + my_tag + '.' + common.utc_YmdHMS() \
            + '.ch' + format(channel, '02d')+ '.coldspace_view' + '.1month'  
    coldspace_view_sql_3day = getcalc_sql(Calc_channel_table_3day,str(channel),'coldspace_view')+ conf.export_txt%(coldspace_view_tmpfile3day + '.txt')
    coldspace_view_sql_1month = getcalc_sql(Calc_channel_table_1month,str(channel),'coldspace_view')+ conf.export_txt%(coldspace_view_tmpfile1month + '.txt')  
    
    tmpfile3day = '/assimilation/fymonitor/DATA/TMP/' + my_tag + '.' + common.utc_YmdHMS() \
            + '.ch' + format(channel, '02d') + '.3day'
    sql_3day = common.get_obc_3dim_sql(ins_conf.obc_3dim_output_db.values(),str(channel),
    ...
    sql_1month = common.get_obc_3dim_sql(ins_conf.obc_3dim_output_db.values(),str(channel),
    ...
    try:
        ...
        cur.execute(blackbody_view_sql_3day)
        cur.execute(blackbody_view_sql_1month)
        cur.execute(coldspace_view_sql_3day)
        cur.execute(coldspace_view_sql_1month)     
    ...
    get_channel_txt(tmpfile1year,tmpfilelife,'ira_calcoef1',channel)
    get_channel_txt(tmpfile1year,tmpfilelife,'ira_calcoef2',channel)
    get_channel_txt(tmpfile1year,tmpfilelife,'ira_calcoef3',channel)
    get_channel_txt(tmpfile1year,tmpfilelife,'blackbody_view',channel)
    get_channel_txt(tmpfile1year,tmpfilelife,'coldspace_view',channel)
    ...
    blackbody_view_data_3day = numpy.loadtxt(blackbody_view_tmpfile3day + '.txt', dtype='str', delimiter=',')
    blackbody_view_tmpfile1month_data_1month = numpy.loadtxt(blackbody_view_tmpfile1month + '.txt', dtype='str', delimiter=',')
    coldspace_view_data_3day = numpy.loadtxt(coldspace_view_tmpfile3day + '.txt', dtype='str', delimiter=',')
    coldspace_view_tmpfile1month_data_1month = numpy.loadtxt(coldspace_view_tmpfile1month + '.txt', dtype='str', delimiter=',')
    ...
    ret = create_obc_hdf('1month',hfile,data_1month, ins_conf.obc_3dim_output_db.values())
    if not ret:
        return False
    
    ymdh_arr = numpy.array(map(common.time_to_arr, data_3day[:, 0]) )
    hfile.create_dataset("time_3day", data = ymdh_arr.astype(numpy.int32))     
    ret = create_obc_hdf('3day',hfile,data_3day, ins_conf.obc_3dim_output_db.values())
    if not ret:
        return False
    
    ymdh_arr = numpy.array(map(common.time_to_arr, blackbody_view_data_3day[:, 3]) )
    hfile.create_dataset("blackbody_view_time_3day", data = ymdh_arr.astype(numpy.int32))
    hfile.create_dataset("blackbody_view_3day", data=blackbody_view_data_3day[: ,5].astype(numpy.float32)) 
    ymdh_arr = numpy.array(map(common.time_to_arr, blackbody_view_tmpfile1month_data_1month[:, 3]) )
    hfile.create_dataset("blackbody_view_time_1month", data = ymdh_arr.astype(numpy.int32))
    hfile.create_dataset("blackbody_view_1month", data=blackbody_view_tmpfile1month_data_1month[: ,5].astype(numpy.float32))
    
    
    ymdh_arr = numpy.array(map(common.time_to_arr, coldspace_view_data_3day[:, 3]) )
    hfile.create_dataset("coldspace_view_time_3day", data = ymdh_arr.astype(numpy.int32))
    hfile.create_dataset("coldspace_view_3day", data=coldspace_view_data_3day[: ,5].astype(numpy.float32)) 
    ymdh_arr = numpy.array(map(common.time_to_arr, coldspace_view_tmpfile1month_data_1month[:, 3]) )
    hfile.create_dataset("coldspace_view_time_1month", data = ymdh_arr.astype(numpy.int32))
    hfile.create_dataset("coldspace_view_1month", data=coldspace_view_tmpfile1month_data_1month[: ,5].astype(numpy.float32))
     
    
    
    
    create_1year_hdf_channel(tmpfile1year,tmpfilelife,'ira_calcoef1',hfile)
    create_1year_hdf_channel(tmpfile1year,tmpfilelife,'ira_calcoef2',hfile)
    create_1year_hdf_channel(tmpfile1year,tmpfilelife,'ira_calcoef3',hfile)
    create_1year_hdf_channel(tmpfile1year,tmpfilelife,'blackbody_view',hfile)
    create_1year_hdf_channel(tmpfile1year,tmpfilelife,'coldspace_view',hfile)
    hfile.close()
...
def get_channel_txt(tmpfile1year,tmpfilelife,type,channel):
    ...
    sql_1year = 'select * from FY3C_IRAS_DAILY_20 where type = \''+type + \
                '\'' + ' and channel = ' + str(channel) +' and ymdh > ' + '\'' +last_year + '\'' + \
                conf.export_txt%(tmpfile1year +'.' +type + '.txt')

    sql_life = 'select * from FY3C_IRAS_DAILY_20 where type = \''+type + \
...
def get_obc_txt(tmpfile1year,tmpfilelife,type):
    ...
    #select * from FY3C_MWTS_DAILY order by ymdh desc limit 365;
    sql_1year = 'select * from FY3C_IRAS_DAILY where type = \''+type + \
                '\'' + ' and ymdh > ' + '\'' +last_year + '\'' + \
                conf.export_txt%(tmpfile1year +'.' +type + '.txt')
    sql_life = 'select * from FY3C_IRAS_DAILY where type = \''+type + '\''\
                + conf.export_txt%(tmpfilelife +'.' +type + '.txt')
...
def create_1year_hdf(tmpfile1year,tmpfilelife,type,hfile):
    ...
    hfile.create_dataset(type+"_avg_1year",data=data_1year[: ,3].astype(numpy.float32)/1000) 
    hfile.create_dataset(type+"_max_1year",data=data_1year[: ,4].astype(numpy.float32)/1000) 
    hfile.create_dataset(type+"_min_1year",data=data_1year[: ,5].astype(numpy.float32)/1000) 
    hfile.create_dataset(type+"_std_1year",data=data_1year[: ,6].astype(numpy.float32)/1000) 
    ...
    hfile.create_dataset(type+"_avg_life",data=data_life[: ,3].astype(numpy.float32)/1000) 
    hfile.create_dataset(type+"_max_life",data=data_life[: ,4].astype(numpy.float32)/1000) 
    hfile.create_dataset(type+"_min_life",data=data_life[: ,5].astype(numpy.float32)/1000) 
    hfile.create_dataset(type+"_std_life",data=data_life[: ,6].astype(numpy.float32)/1000) 
...
#         + conf.export_txt%(tmpfile1month + '.txt')
    ...
    begin_sql_3day_calc = conf.obc_select_prefix_sql + ' 1 from ' + Calc_table_3day[0] \
                + " limit 1"
...
#         cur.execute(sql_1month)
        ...
        cur.execute(begin_sql_3day_calc)
        begin_data = cur.fetchall()
        cur.close()
        conn.close()
    except MySQLdb.Error, e:
...
def main():
    ...
    Calc_channel_tag = 'OBCXX_MS_CALC_' + str(ins_conf.channels)
    Calc_channel_table = [ x for x in all_obc_table if Calc_channel_tag in x[0]]
    
    Calc_table = list(set(Calc_table).difference(set(Calc_channel_table)))
...
#############################################################
    # sort by filename time asc.
    Calc_channel_table = sorted(Calc_channel_table)
    now_time = datetime.now()
    shifen = now_time.strftime("%H%M")
    ...
    if int(shifen)< 0030:
        for i in range(0, 3):
            ...
            for idx, one_table in enumerate(Calc_channel_table):
                if ymd in one_table[0]:
                    Calc_channel_table_3day.extend([one_table[0]])
    else:
        for i in range(0, 3):
            #yes_time = now_time + datetime.timedelta(days=(-2 + i))
            yes_time = now_time + timedelta(days=(-2 + i))
    ...
    for i in range(0, 30):
        ...
        for idx, one_table in enumerate(Calc_channel_table):
            if ymd in one_table[0]:
                Calc_channel_table_1month.extend([one_table[0]])
